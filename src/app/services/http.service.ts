...
export class HttpService {
  ...
  private readonly imagesBaseUrl = 'https://cdn.jsdelivr.net/gh/akabab/superhero-api@0.3.0/api/images';

  findAll(): Observable<SuperHero[]> {
    return this.http.get<SuperHero[]>(`${this.baseUrl}/all.json`);
  }

  findById(id: number): Observable<SuperHero> {
  ...
  findPowerStatsById(id: number): Observable<SuperHero['powerstats']> {
    return this.http.get<SuperHero['powerstats']>(`${this.baseUrl}/id/${id}.json`);
  }

  findAppearanceById(id: number): Observable<SuperHero['appearance']> {
    return this.http.get<SuperHero['appearance']>(`${this.baseUrl}/id/${id}.json`);
  }

  findBiographyById(id: number): Observable<SuperHero['biography']> {
    return this.http.get<SuperHero['biography']>(`${this.baseUrl}/id/${id}.json`);
  }
  ...
  findConnectionsById(id: number): Observable<SuperHero['connections']> {
    return this.http.get<SuperHero['connections']>(`${this.baseUrl}/id/${id}.json`);
  }
